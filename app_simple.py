...
async def conduct_research(question: str):
    ...
    try:
        ...
        # Check if there are follow-up questions and create numbered list
        follow_ups = extract_follow_up_questions(output)
        if follow_ups:
            await create_follow_up_list(follow_ups)
...
async def handle_follow_up_selection(selection: int):
    ...
    if session_id in session_follow_ups and selection in session_follow_ups[session_id]:
        ...
        await cl.Message(
            content=f"""❓ I couldn't find follow-up question #{selection}. 
...
def format_research_output(output: str) -> str:
    """Format the research output for better display"""
...
async def create_follow_up_list(follow_ups: list):
