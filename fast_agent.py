...
class MCPAgentManager:
    ...
    def _get_llm(self):
        """Get the configured Cerebras LLM"""
        # Get model selection from user session or use default
        model_name = cl.user_session.get("cerebras_model", "qwen-3-235b-a22b-thinking-2507")
        return ChatCerebras(
            model=model_name,
            temperature=0.0,
            max_tokens=15000,
...
🤖 **Model**: Cerebras Inference qwen-3-235b-a22b-thinking-2507 optimized for research
...
"""
    ...
    cl.user_session.set("cerebras_model", "qwen-3-235b-a22b-thinking-2507")
...
async def on_settings_update(settings):
    ...
    if "gemini_model" in settings:
        cl.user_session.set("cerebras_model", settings["cerebras_model"])
        ...
        await cl.Message(
            content=f"✅ Cerebras model updated to: **{settings['cerebras_model']}**"
        ).send()
